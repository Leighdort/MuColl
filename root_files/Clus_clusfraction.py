#This is seeing how much energy the clusters differ by
#Let's print out some mid and some extreme differences 
...
for num in energies:
    ...
    for i in range(events.num_entries):
        ...
        if len(cluster_energy[i]) == 1:
            ...
            cy_n = cluster_y[i][0]
            cz_n = cluster_z[i][0]
            nobib_energy = cluster_energy[i][0]
            nobib_r = np.sqrt(cx_n**2 + cy_n**2 + cz_n**2)
            cn_theta = np.arccos(cz_n / nobib_r)
            cn_phi = np.arctan2(cy_n, cx_n)
            cos_nang = (np.sin(mc_theta)*np.sin(cn_theta)*np.cos(mc_phi - cn_phi)+ np.cos(mc_theta)*np.cos(cn_theta))
            cos_nang = np.clip(cos_nang, -1.0, 1.0)
            nobib_distance = np.arccos(cos_nang)
            ...
            fraction_clus = max_energy / nobib_energy
            passes += 1
            fraction.append(fraction_clus)
    ...
    plt.xlabel(f"Bib Energy/Nobib Energy Fraction")
    plt.ylabel("Count")
    plt.axvline(
        median,
        color='red',
    ...
    plt.legend()
    plt.title(f"Fraction bib/nobib given 1 Cluster within Bounds Electrons {num}")
    plt.tight_layout()
    plt.savefig(f"2mc_fraction_bib_elec{num}GeV.pdf")
    plt.close()

#Third summary graph, number of bib clusters given 1 normal cluster
plt.errorbar(choices, mc_median, yerr=[mc_low, mc_high], fmt='s', alpha= 0.6, capsize=4, label="Electron particle Gun")
plt.xlabel("Beam Energy")
plt.ylabel("Median Clusterbib/Cluster Energy")
plt.title("Median Clusterbib/Cluster Energy (Electron) Given 1 BibCluster within Bounds")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig("2summary_clus_electron_bib.pdf")
plt.close()