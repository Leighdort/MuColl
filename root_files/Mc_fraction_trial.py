...
for pid in particle:
    ...
    for a in angles:
        ...
        plt.legend()
        plt.title(f"Trial10.2 {a} deg given 1 Cluster PID: {pid} Nobib Energy 50 GeV")
        plt.tight_layout()
        plt.savefig(f"trial10.2_{a}_fraction_nobib_{pid}50GeV2.pdf")
        plt.close()
        '''
        ...
        cal.append(median)
        #Now get new plots based on new calibration: 
        '''
...
#plt.errorbar(angles, cal, yerr=[cal_low, cal_high], fmt='s', alpha= 0.6, capsize=4, label=f"Ratio Calibration {pid}")
plt.xlabel("Particle gun angle (degrees)")
plt.ylabel("Median Response")
plt.title(f" Trial 10.2 50 GeV Median Cluster/MC Energy 1 Nobib Cluster Pid: {pid} ")
#plt.title(f" Original 50 GeV Median Response 1 Nobib Cluster Pid: {pid} ")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig(f"trial10_new_{pid}2.pdf")
plt.close()
