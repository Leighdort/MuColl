...
for pid in particle:
    for e in energy: 
        ...
        for a in angles:
            ...
            file = uproot.open(f"/users/rldohert/data/mucoll/rldohert/pdg_{pid}_pt_{e}_theta_{a}-{a}_trial10/job_0/reco_output_p{e}_{pid}_nobib0.edm4hep.root")
            ...
            ecal_energy_ratio = np.array(ecal_energy_ratio)
            hcal_energy_ratio = np.array(hcal_energy_ratio)
            if pid not in results:
                results[pid] = {}
            if a not in results[pid]:
                results[pid][a] = {
                    "energy": [],
                    "median": [],
                    "low": [],
            ...
            if pid not in results_4cal:
                results_4cal[pid] = {}
            if a not in results_4cal[pid]:
                results_4cal[pid][a] = {
                    "energy": [],
                    "median": [],
                    "low": [],
            ...
            #THis is the calibration for the 4 Cal number thing
            q16, q84 = np.percentile(fraction, [16, 84])
            median_full = np.median(fraction)
            results_4cal[pid][a]["energy"].append(e)
            results_4cal[pid][a]["median"].append(median_full)
            results_4cal[pid][a]["low"].append(median_full - q16)
            results_4cal[pid][a]["high"].append(q84 - median_full)
            ...
            q16, q84 = np.percentile(fraction, [16, 84])
            median_full = np.median(fraction)
            results[pid][a]["energy"].append(e)
            results[pid][a]["median"].append(median_full)
            results[pid][a]["low"].append(median_full - q16)
            results[pid][a]["high"].append(q84 - median_full)
            ...
            if (a >= 320) or (a <= 40) or (140 <= a <= 220): #endcap
                for i in range(len(fraction)):
                    baseline = fraction[i]
                    correction_1 = ecal_energy_ratio[i] * .11
                    correction_2 = hcal_energy_ratio[i] * .11
                    new_fraction = baseline - correction_1 - correction_2
                    fraction[i] = new_fraction
            else: #barrel
                for i in range(len(fraction)):
                    baseline = fraction[i]
                    correction_1 = ecal_energy_ratio[i] * .06
                    correction_2 = hcal_energy_ratio[i] * .11
                    new_fraction = baseline - correction_1 - correction_2
                    fraction[i] = new_fraction
            print(len(fraction))
            ...
            plt.ylabel("Count")
            plt.xlim(0, 2)
            plt.title(f"Response ({pid}, {a}°, {e} GeV) Base, NoSoftware, Split")
            plt.legend()
            plt.tight_layout()
            plt.savefig(f"combined_gaussian_{a}_{pid}_{e}_base_nosoftware.pdf")
            plt.close()
            
            #Let's just plot fraction now w/ energy
            
            '''
            #Now we are doing an Energy Split Graph 
            bins = np.linspace(0, 2, 60)
            plt.figure(figsize=(7,5))
            median = np.median(fraction)
            # Below 0.5
            ...
            plt.axvline(
                ...
                linestyle='--',
                linewidth=2,
                label=f'Below 0.5 median = {median_below_e:.3f}'
            )
            ...
            plt.axvline(
                ...
                linestyle='--',
                linewidth=2,
                label=f'Above 0.5 median = {median_above_e:.3f}'
            )
            ...
            plt.ylabel("Count")
            plt.xlim(0, 2)
            plt.title(f"Response ({pid}, {a}°, {e} GeV) Baseline-Split Energy Calib")
            plt.legend()
            plt.tight_layout()
            plt.savefig(f"combined_gaussian_{a}_{pid}_{e}_baseline_energy_calib2.pdf")
            plt.close()
            '''
            ...
            plt.xlim(0, 2) #normally 0.6 to 1.4 30 bins 
            ...
            plt.legend()
            plt.title(f"Response ({pid}, {a}°, {e} GeV) BaseNoSoftware")
            plt.tight_layout()
            plt.savefig(f"full_gaussian_{a}_{pid}_{e}_base_nosoftware.pdf")
            plt.close()
        #This will be a gaussian 
        #1 line for each angle
        #a gaussian for each pid 
        #Vary through energy
#Now we need the other thing too here
#This is Graphing the Full Thing
for pid in particle:
    for e in energy: 
        mc_median= []
        mc_low = []
        ...
        for a in angles:
            ...
            ecal_energy_ratio = np.array(ecal_energy_ratio)
            hcal_energy_ratio = np.array(hcal_energy_ratio)
            if pid not in results_base:
                results_base[pid] = {}
            if a not in results_base[pid]:
                results_base[pid][a] = {
                    "energy": [],
                    "median": [],
                    "low": [],
            ...
            q16, q84 = np.percentile(fraction, [16, 84])
            median_full = np.median(fraction)
            results_base[pid][a]["energy"].append(e)
            results_base[pid][a]["median"].append(median_full)
            results_base[pid][a]["low"].append(median_full - q16)
            results_base[pid][a]["high"].append(q84 - median_full)
...
for pid in particle:
    ...
    for a in angles:
        E = results[pid][a]["energy"]
        med = results[pid][a]["median"]
        low = results[pid][a]["low"]
        high = results[pid][a]["high"]
        E_base = results_base[pid][a]["energy"]
        med_base = results_base[pid][a]["median"]
        low_base = results_base[pid][a]["low"]
        high_base = results_base[pid][a]["high"]
        E_4cal = results_4cal[pid][a]["energy"]
        med_4cal = results_4cal[pid][a]["median"]
        low_4cal = results_4cal[pid][a]["low"]
        high_4cal = results_4cal[pid][a]["high"]
        ...
        plt.errorbar(
            ...
            marker='o',
            capsize=4,
            label=f"{angle2region[a]} 4Cal/HitCal"
        )
        plt.errorbar(
            E_base,
            med_base,
            yerr=[low_base, high_base],
            marker='o',
            capsize=4,
            label=f"{angle2region[a]} Baseline"
        )
        plt.errorbar(
            E_4cal,
            med_4cal,
            yerr=[low_4cal, high_4cal],
            marker='o',
            capsize=4,
            label=f"{angle2region[a]} 4Cal"
        )

    plt.xlabel("Energy [GeV]")
    plt.ylabel("Median Response")
    plt.title(f"Median Response for PID {pid} ")
    plt.legend()
    plt.grid(True)
    plt.savefig(f"response_vs_energy_pid_{pid}_three.pdf")
    plt.close()
...
for pid in particle:
    ...
    for a in angles:
        E = np.array(results[pid][a]["energy"])
        med = np.array(results[pid][a]["median"])
        low = np.array(results[pid][a]["low"])
        high = np.array(results[pid][a]["high"])
        sigma = (high + low)/2
        resolution = sigma / med 
        E_base = np.array(results_base[pid][a]["energy"])
        med_base = np.array(results_base[pid][a]["median"])
        low_base = np.array(results_base[pid][a]["low"])
        high_base = np.array(results_base[pid][a]["high"])
        sigma_base = (high_base + low_base)/2
        resolution_base = sigma_base / med_base
        E_4cal = np.array(results_4cal[pid][a]["energy"])
        med_4cal = np.array(results_4cal[pid][a]["median"])
        low_4cal = np.array(results_4cal[pid][a]["low"])
        high_4cal = np.array(results_4cal[pid][a]["high"])
        sigma_4cal = (high_4cal + low_4cal)/2
        resolution_4cal = sigma_4cal / med_4cal
        plt.plot(
            E,
            resolution,
            marker='o',
    ...
    plt.xlabel("Energy [GeV]")
    plt.ylabel(r"Resolution $\sigma/E$")
    plt.title(f"Resolution for PID {pid}")
    plt.legend()
    plt.grid(True)
    plt.savefig(f"resolution_vs_energy_pid_{pid}_three.pdf")
    plt.close()
