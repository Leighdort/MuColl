...
for num in energies:
    ...
    '''
    print(f"\n--- Event display candidates for {num} GeV Electron---")
    for target in targets:
        dists = np.abs(fraction - target)
        closest = np.argsort(dists)[:5]
        print(f"  Near fraction {target:.2f}:")
        for idx in closest:
            print(f"    Event {fraction_events[idx]}, fraction={fraction[idx]:.3f}")
        fraction = np.array(fraction)
    '''
    bins=30
    plt.hist(fraction, bins=bins, range = (0,2), edgecolor = 'black')
    plt.xlabel(f"Cluster/MC Energy Fraction with Bib")
    plt.ylabel("Count")
    plt.axvline(
        median,
        color='red',
    ...
    plt.legend()
    plt.title(f"Fraction given 1 Cluster within Bounds Neutrons Bib {num}")
    plt.tight_layout()
    plt.savefig(f"mc_fraction_bib_neutron{num}GeV.pdf")
    plt.close()


#Let's make a resolution plot:
resolution = [(high + low) / 2 / med for high, low, med in zip(mc_high, mc_low, mc_median)]
plt.errorbar(choices, resolution, fmt='s', alpha=0.6, capsize=4, label="Neutron particle gun")
plt.xlabel("Beam Energy")
plt.ylabel("Resolution")
plt.yticks([0.05, 0.075, 0.10, 0.125, 0.15, 0.175, 0.20, 0.225])
plt.title("Resolution for reconstructed Energy Neutrons Bib")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig("summary_bib_neutrons_res.pdf")
plt.close()
...
#uscms-pursue-commitee@fnal.gov.
