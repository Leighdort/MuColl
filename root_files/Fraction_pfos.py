#This is checking the PFOS reconstruction energy
...
for num in choices:
    ...
    pfo_energy = pandora_pfos["PandoraPFOs.energy"].array()
    print(f"Processing {num} GeV")
    fraction_for_this_energy = []
    for i in range(events.num_entries):
        status = statuses[i]
        pfo_energies = pfo_energy[i]
        mass = masses[i]
        ...
        pfo_total = np.sum(pfo_energies)
        momentum = np.sqrt(mx**2 + my**2 + mz**2)
        mc_energy = np.sqrt(m*m + momentum*momentum)
        if pfo_total is None or mc_energy is None:
            continue
        fraction = pfo_total / mc_energy
    ...
    plt.xlabel("Ratio of all PFOs to MC Energy")
    plt.ylabel("Count")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"pfos_ratio_electrons{num}GeV.pdf")
    plt.close()
...
for num in choices:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"pfos_ratio_pions{num}GeV.pdf")
    plt.close()
...
plt.ylabel("Pfos/MC Particle")
plt.title("Pfos/MC Particle per energy")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig("summary_fraction_pfos.pdf")
plt.close()


#I should also plot them all on the same graph 
##Have that be able to be turned on and off