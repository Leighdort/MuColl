...
for pid in particle:
    ...
    for a in angles:
        ...
        #Now getting pfo ids:
        pandora_pfos = events["PandoraPFOs"]
        pandora_pdg = pandora_pfos["PandoraPFOs.PDG"].array()
        angular_dist = []
        regular_dist = []
        ...
        print("pfos entries:", len(pandora_pfos.array()))
        for i in range((events.num_entries)):
            ...
            if len(cluster_energy[i]) == 1:
                ...
                pandora_list = pandora_pdg[i] #this literally just worked: 
                if yes == True: 
                    #print("hi")
                    passes += 1
                    ...
                    if (ecal_total / total) >= 0.5:
                        pfo_above.append(pandora_list)
                    if (ecal_total / total) < 0.5:
                        pfo_below.append(pandora_list)
                    pfos_all.append(pandora_list) #this may or may not be a list 
        fraction = np.array(fraction)
        ecal_ratio = np.array(ecal_ratio)

        # ---- FIX PFO FLATTENING ----
        pfos_all = np.concatenate([np.asarray(x) for x in pfos_all])
        pfos_all = np.asarray(pfos_all)
        pfo_above = np.concatenate([np.atleast_1d(x) for x in pfo_above])
        pfo_below = np.concatenate([np.atleast_1d(x) for x in pfo_below])
        pfo_above = np.asarray(pfo_above)
        pfo_below = np.asarray(pfo_below)

        # ---- UNIQUE COUNTS ----
        unique_pfos, counts = np.unique(pfos_all, return_counts=True)
        plt.figure(figsize=(8, 5)) 
        plt.bar(unique_pfos.astype(str), counts)
        plt.xlabel("PFO type (ID)")
        plt.ylabel("Count")
        plt.title(f"PFOS distribution for {a} and {e} for {pid}")
        plt.tight_layout()
        print(f"hi")
        plt.savefig(f"pfos_{a}_{e}_{pid}.pdf")
        plt.close()

        unique_pfos, counts = np.unique(pfo_above, return_counts=True)
        plt.figure(figsize=(8, 5)) 
        plt.bar(unique_pfos.astype(str), counts)
        plt.xlabel("PFO type (ID)")
        plt.ylabel("Count")
        plt.title(f"PFOS distribution for {a} and {e} for {pid} >= 0.5 in Ecal")
        plt.tight_layout()
        print(f"hi")
        plt.savefig(f"pfos_{a}_{e}_{pid}_above.pdf")
        plt.close()

        unique_pfos, counts = np.unique(pfo_below, return_counts=True)
        plt.figure(figsize=(8, 5)) 
        plt.bar(unique_pfos.astype(str), counts)
        plt.xlabel("PFO type (ID)")
        plt.ylabel("Count")
        ...
        plt.tight_layout()
        print(f"hi")
        plt.savefig(f"pfos_{a}_{e}_{pid}_below.pdf")
        plt.close()
