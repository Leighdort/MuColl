...
for num in energies:
    ...
    passes = 0
    if num == 2:
        #Bound goes pion, electron, neutron
        #bound = 0.05
        #bound = 0.01
        bound = 0.056
    if num == 10:
    ...
    print(f" Electron Number of 1 Event Clusters: {num_events_1_clus}")
    print(f" Electron Number of Events that fall within margin: {passes}")
    fraction = np.array(fraction)
    bins=30
    #edges = 0 and 2 
    plt.hist(fraction, bins=bins, range=(.85, 1.1), edgecolor='black')
    ...
    plt.legend()
    plt.title(f"Fraction given 1 Cluster within Bounds Electron Nobib {num}")
    plt.tight_layout()
    plt.savefig(f"mc_fraction_nobib_neutron4{num}GeV.pdf")
    plt.close()
...
plt.title("Resolution for reconstructed Energy Neutrons Nobib")
plt.grid(True)
ax.set_ylim(0.05, 0.255)
ax.set_yticks([0.05, 0.075, 0.10, 0.125, 0.15, 0.175, 0.20, 0.225, 0.25])
plt.legend()
plt.tight_layout()
plt.savefig("summary_nobib_neutron3_res.pdf")
plt.close()
