...
Run_bib.sh is currently hardcoded to only add bib to one set of parameters in one folder at a time. This can be changed. It takes in the sim files that you previously wrote. However, mine currently does not do that as I hit the oscar batch limit at 1000. If you would like to not go through the entire gen/sim/digi/reco, you can just sub in the digi section into the full_chain.sh. Take special care to make sure your bib paths are correct. If bib doesn't run you may need to overwrite the following ( CKT: https://github.com/samf25/mucoll-benchmarks/blob/k4MuC/reconstruction/reco_components/CKF_tracking.py#L103-L124). Otherwise it should run. Take notice that the random seed changes every time (this is important)!


Another sidenote, currently logs/error appears both where the run_bib.sh file is and in the data directory. This might annoy you. This can be changed, but I haven't gotten to it yet (it doens't bother me too much). 
